...
**Corpus** — 526 puzzles across 5 series (Guardian cryptic 195, Independent Sunday 95, Independent daily 80, Observer Everyman 79, Guardian Quiptic 77). 361 of them are annotated clue-for-clue, all six rungs, machine-validated: 10,586 of 15,372 clues, or 69%. The rest are backlog the daily job is still draining — they show an <em>auto hints</em> badge and degrade gracefully (checking and letter reveals still work; the teaching ladder appears once a puzzle is annotated).
<!-- CORPUS-END -->
...
**Where it runs:** both timed jobs are lines in the bridge container's `tools/crontab`
(the `household` repo) — 06:15 for this script, `:05` hourly for `prereset_backfill.sh`,
each behind `flock -n`. That crontab is the only schedule either job has, and it is the
only one either job may have: a second scheduler is not a fallback, because both copies
would annotate the same backlog out of the same weekly quota. No LaunchAgent is shipped
here for that reason.

The container reads its login from a file under `CLAUDE_CONFIG_DIR`, which is what makes
cron viable at all. **On macOS it is not.** The `claude` CLI stores its OAuth credentials
in the *login* keychain (item `Claude Code-credentials`); `cron` runs outside the GUI
login session and cannot unlock it, so every run dies with `Not logged in · Please run
/login` and the backlog never drains. Scheduling this on a Mac means a per-user
LaunchAgent bootstrapped into `gui/<uid>`, written by hand — and deleting the crontab
lines as you add it, not running both. (Related trap: cron's bare `PATH` also hides
`~/.local/bin/claude`, which is why the script sets an explicit one.)

Anywhere credentials live in `~/.claude/.credentials.json` rather than a keychain, the
whole schedule is two lines:
