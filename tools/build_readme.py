...
LAYOUT = [
    ...
    ("fetching", "tools/repair_fetched.py", "applies the fetchers’ current rules to puzzles already on disk — bare-capital solutions, a masked prize answer, a linked group whose members disagree — and reports the dates their own neighbours contradict"),
    ("fetching", "tools/test_repair_fetched.sh", "builds a puzzle file with each of those defects and proves the repair fixes it, leaves a clean file byte-identical, and is clean on the second run"),
