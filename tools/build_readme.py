...
LAYOUT = [
    ...
    ("fetching", "tools/test_ia_borrow.sh", "holds the one archive.org refusal string that means two different things apart, so a book whose copies are all out is never read as one that needs no loan"),
