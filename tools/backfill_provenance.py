...
file's own record of its acquirer and git's record of when the puzzle first
appeared, so a second run rewrites the same bytes and reports 0 changed. That matters because
...
# The suffixes a puzzle has been committed under. History is read by id and
# never by path: a puzzle is one puzzle whichever form holds it, and keying on
# the path makes a change of form read as the whole corpus arriving at once,
# which would stamp that one day on every acquiredOn in the repo.
...
def add_dates():
    """puzzle id -> the date it first appears, in ONE traversal of the history.

    Keyed by id rather than by path, because a puzzle is committed under one
    suffix at a time: the form it is stored in changing gives every file a
    fresh "A" under its new path, and reading those as arrivals would date the
    whole corpus to that one day. Both suffixes collapse onto the id, so the
    oldest date still wins.
    ...
    for line in out.splitlines():
        ...
        pid = puzzle_id(line)
        if pid:
            dates[pid] = date       # newest first, so the last write is the oldest
    return dates
...
def main(argv=None):
    ...
    for path in puzzle_files():
        ...
        changed += 1
        if not args.dry_run:
            # The acquirer is preserved, not restamped: this tool fills a field
            # in a file a fetcher laid out, and putting its own name on top
            # would destroy the very acquisition record it is here to write
            # down.
