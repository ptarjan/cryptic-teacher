...
# is not a puzzle) and update it. 23 = cryptic at five sizes, cyclops at two,
# and book, everyman, ftcryptic, globeandmail, independent, indysunday, metro,
# quiptic, sundaytel, sundaytimes, sundaytough, telegraph, times, timesjumbo,
# timesquick and toughie at one each. Every scanned printed book is the one `book` series, so a
# thirty-first book does not move this number.
same "every series and size was drawn from" "$(field GROUPS "$out2")" "23"
