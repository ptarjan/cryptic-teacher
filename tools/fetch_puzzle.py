...
def generator_of(path):
    """The tool the file's banner already names, or the default for a new file.

    The banner records what ACQUIRED the puzzle, which is a fact about the
    fetcher and not about whichever tool most recently rewrote the annotations
    inside it. Restamping it destroys the acquisition record — and did: four
    files ended up claiming apply_solution.py, and blind_annotate.py's restore
    path put the plain default back over whatever a non-Guardian fetcher had
    written. Preserving is therefore the DEFAULT below, so a caller cannot
    silently relabel a puzzle by forgetting an argument; a fetcher that really
    is the acquirer says so explicitly.
    """
    if not path.exists():
