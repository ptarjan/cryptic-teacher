...
Usage:
  ...
  python3 tools/fetch_puzzle.py --latest         # newest of EVERY series (see SERIES)
...
# Three independent series, each with its own number sequence.
#
...
#
# The three number ranges are ~30,000, ~1,400 and ~4,100, and all count upward,
# so a bare number is still unambiguous and puzzle ids / filenames / URLs stay
# plain integers. That is deliberate: prefixing ids by series would have moved
# nothing but would have forced every existing /puzzles/<n>/ page and stored
# progress key to grow a special case. The sequences are far enough apart that
# the first collision (everyman reaching 30,000) is ~500 years out.
SERIES = {
    "cryptic": {
        "label": "Cryptic",
    ...
    "everyman": {
        ...
        # The Guardian ships no creator for these, because "Everyman" IS the
        # byline — the Observer has kept the setter anonymous since 1945. Without
        # this the converter's fallback labels every one of them "Unknown", which
        # reads as a scraping failure rather than the deliberate anonymity it is.
        "setter": "Everyman",
    },
}
...
def series_of(page_id):
    """Which SERIES a puzzle belongs to, read off the page's own id
    ...
    Everything else lives under its own name. Written as a scan over SERIES so
...
def convert(data):
    ...
    return {
        ...
        "setter": ((data.get("creator") or {}).get("name")
                   or SERIES[series_of(data["id"])].get("setter", "Unknown")),
        "date": data.get("date"),
