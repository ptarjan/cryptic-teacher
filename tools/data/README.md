...
| `books.json` | The shelf: one row per physical book, carrying its archive.org identifier, the volume number its own cover prints, the shelf label a reader sees, the publisher, the printed title and a `book_index` that is never reused. `tools/series.py` reads it and `tools/fetch_puzzle.py --reindex` copies the browser's half into `puzzles/index.json` for `app.js`; neither holds a copy of a row. See the `README` inside it. |
...
| `blog_facts/` | Per series, what each puzzle's blog write-up (timesforthetimes, fifteensquared, bigdave44) marks about its clues: the underlined definition, a clue type the blogger named, marked indicators, and the post's URL. Words of the clues themselves, never the blog's prose. Written by `tools/blog_facts.py` from the local caches, which CI does not have, so it is committed; the shims merge it in (see APP.md, "Hints read off a blog"). |
